...
object TimeUsage extends TimeUsageInterface {
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    ...
    val primaryNeedsColumnsName = columnNames.filter(name => name.startsWith("t01")
      || name.startsWith("t03")
      || name.startsWith("t11")
      || name.startsWith("t1801")
      || name.startsWith("t1803"))

    val workingActivitiesColumnsName = columnNames.filter(name => name.startsWith("t05")
      || name.startsWith("t1805"))

    val otherActivitiesColumnsName = columnNames.filter(name => (name.startsWith("t02")
      || name.startsWith("t04")
      || name.startsWith("t06")
      || name.startsWith("t07")
      || name.startsWith("t08")
      ...
      || name.startsWith("t16")
      || name.startsWith("t18"))
      && !primaryNeedsColumnsName.contains(name)
      && !workingActivitiesColumnsName.contains(name))

    val primaryNeedsColumns = primaryNeedsColumnsName.map(col(_))
    val workingActivitiesColumns = workingActivitiesColumnsName.map(col(_))
    val otherActivitiesColumns = otherActivitiesColumnsName.map(col(_))
  ...
  def timeUsageSummaryTyped(timeUsageSummaryDf: DataFrame): Dataset[TimeUsageRow] = {
    timeUsageSummaryDf.map(row => {
      val col1 = row.getAs[String]("working")
      val col2 = row.getAs[String]("sex")
      val col3 = row.getAs[String]("age")
      val col4 = row.getAs[Double]("primaryNeeds")
      val col5 = row.getAs[Double]("work")
      val col6 = row.getAs[Double]("other")
      TimeUsageRow(col1, col2, col3, col4, col5, col6)
    })
  }
  ...
  def timeUsageGroupedTyped(summed: Dataset[TimeUsageRow]): Dataset[TimeUsageRow] = {
    ...
    val ds = summed.groupByKey(key => (key.working, key.sex, key.age))
      .agg(typed.avg(arg => arg.primaryNeeds),
        typed.avg(arg => arg.work),
        typed.avg(arg => arg.other))
      .map { case ((working, sex, age), avgPrimaryNeeds, avgWork, avgOther)
      => TimeUsageRow(working, sex, age, avgPrimaryNeeds, avgWork, avgOther)}

    val df = ds.toDF("working", "sex", "age", "primaryNeeds", "work", "other").
      select($"working", $"sex", $"age",
        round($"primaryNeeds", 1).as("primaryNeeds"),
        round($"work", 1).as("work"),
        round($"other", 1).as("other")).
      orderBy("working", "sex", "age")
